...
window.renderComp=function(){
  ...
  if(!compData.length){
    var defaultComps=[
      ...
      {comp:'Balde',vidaUtil:4000,costoRef:2055147}
    ];
  ...
  var calendarioHtml=!bucketsConDato.length?'':(
    ...
    bucketsConDato.map(function(k){
      ...
      return '<tr'+(k==='Vencido'?' style="background:rgba(239,68,68,.06)"':'')+'>'+
        ...
        '<td class="mono" style="color:var(--ac)">$'+fn(Math.round(b.costo))+'</td>'+
  ...
  $('s-comp').innerHTML=
    ...
    '<div class="card"><div class="card-t">Costo Próximos Cambios</div><div class="card-v" style="color:var(--ac)">$'+fn(Math.round(costoTotal))+'</div><div class="card-s">Componentes &lt;2000h restantes</div></div>'+
    ...
    fil.map(function(c,idx){
      ...
      return'<tr style="'+(cd&&c.hrsRest<=0?'background:rgba(239,68,68,.06)':(cd?'':'opacity:.72'))+'">'+
        ...
        '<td style="font-size:10px">'+(c._histInfo?'<span title="'+escapeHtml(c._histInfo.obs||'')+' ('+escapeHtml(c._histInfo.fuente||'')+')" style="color:var(--tx2);cursor:help">'+escapeHtml(c._histInfo.fechaInst)+(c._histInfo.horomInstalacion!=null?' · '+fn(Number(c._histInfo.horomInstalacion))+'h':'')+'</span>'+(!c.fechaInst&&!orig?' <button class="btn-x" style="font-size:9px;padding:1px 4px" onclick="edComp('+realIdx+',\'fechaInst\',\''+c._histInfo.fechaInst+'\');'+(c._histInfo.horomInstalacion!=null?'edComp('+realIdx+',\'horomComp\','+c._histInfo.horomInstalacion+');':'')+'" title="Completar Fecha Inst. (y Horóm. Inst. si está disponible) con este dato del historial">usar ↑</button>':''):dash)+'</td>'+
