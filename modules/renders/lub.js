...
export function renderLub() {
  ...
  $('s-lub').innerHTML =
    ...
    '<button class="btn" onclick="addLub()">+ Nuevo</button> <button class="btn btn-o" onclick="importLubCSV()"><svg viewBox="0 0 20 20" width="13" height="13" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><polyline points="6,6 10,2 14,6"/><line x1="10" y1="2" x2="10" y2="12"/><polyline points="3,15 3,17 17,17 17,15"/></svg> Importar CSV</button> <button class="btn btn-o" onclick="resumenFlotaLub()">📊 Resumen</button> <button class="btn" style="background:var(--warn,var(--warn));color:#000" onclick="diagVinculos()"><svg viewBox="0 0 20 20" width="15" height="15" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"><circle cx="8.5" cy="8.5" r="5.5"/><line x1="12.7" y1="12.7" x2="17.5" y2="17.5"/></svg> Vínculos rotos</button></div>' +
    '<div class="toolbar">' +
    ...
    pg.items.map(function (l, fi) {
      ...
      return '<tr' + (obsoleto ? ' style="opacity:.55"' : '') + '>' +
        ...
        '<td><button class="btn-s" onclick="verTendenciaLub(' + idx + ')" title="Tendencia y proyección de compra">📈</button></td>' +
