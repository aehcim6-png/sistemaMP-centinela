    ...
    <div class="tbl-wrap"><table>
      ...
      ${pg.items.map((n,_)=>{
        ...
        return`<tr style="${cambiarYa?'background:rgba(239,68,68,.05)':proxCambio?'background:rgba(234,179,8,.04)':''}">
          ...
          <td style="font-size:10px;color:${proyCol}" title="${proy?'Motivo: '+escapeHtml(proy.motivoCambio)+' · Confianza: '+escapeHtml(proy.confianza)+(proy.r2Desgaste!=null?' · Ajuste de la recta de desgaste (R²): '+proy.r2Desgaste+(proy.r2Desgaste<0.5?' (débil — la recta explica poco de los datos reales, la fecha puede no ser confiable)':' (bueno)'):''):'Sin mediciones suficientes para proyectar'}">${proy?'≈'+proy.diasRestantes+'d ('+fn(proy.hrsRestantes)+'h)'+(proy.r2Desgaste!=null&&proy.r2Desgaste<0.5?' ⚠️':''):'—'}</td>
