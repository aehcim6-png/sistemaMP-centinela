...
window.renderAyuda=function(){
  ...
  $('s-ayuda').innerHTML=
    ...
    '<li><b>Mejor clasificación de correctivos por componente:</b> el campo "componente" viene vacío en casi todos los correctivos reales — el sistema lo deriva del texto libre de "síntoma" que sí escribe el técnico. Se amplió ese diccionario en 9 pasadas con evidencia real (medida contra los 1.243 correctivos reales: subió de 42% a 69.3% de cobertura) — ahora reconoce, por ejemplo, "falla eléctrica" (antes solo "falla eléctrico"), presión de neumáticos sin decir la palabra "neumático", "suspencion" con "c", cañerías y flexibles como Mangueras/Fugas, entrecalzas, ripper y canilleras como GET/Cuchillas, "garra maestra" y "sproket" como Tren de Rodaje, y separa Engrase/Lubricación, Fuga de Aceite, Radio/Comunicaciones, Tornamesa/Giro, Sistema AFEX, Estanque/Tapa de Combustible, Correas, Sistema Anticolisión/Fatiga (ADAS), Joystick/Palanca de Mando, Parabrisas/Vidrios, Tolva/Dumper, Puertas, Espejos, Baliza/Pértiga y Cámara de Retroceso como categorías propias. Algunos términos (ej. "F&S", "check point", "viscoso") no se lograban interpretar solo con el texto — se confirmaron directamente con el equipo de mantención antes de agregarlos. Términos sin ninguna evidencia real en la base (ej. "lainas", "bomba centrífuga", "plumilla", "fusible", "carrilera") deliberadamente NO se agregaron — términos que cruzan demasiados sistemas distintos (sensor, válvula, "palanca" genérica) se dejaron sin clasificar en vez de forzarlos, y "pala" (motoniveladora) se descartó a propósito por ser prefijo de "palanca" — agregarla habría clasificado mal cada mención de "palanca". Del resto de correctivos sin clasificar, la mayoría son genuinamente casos sin componente (mantenimiento preventivo, cierre de backlog, etc.) — el resto ya es una cola de síntomas únicos donde seguir sumando exige revisar caso a caso con el equipo de taller, ya no con patrones estadísticos. Se ve reflejado en Estadística → Por Componente y en Predictivo (detección de fallas repetidas).</li>'+
    '</ul>'+
    '</div>'+
